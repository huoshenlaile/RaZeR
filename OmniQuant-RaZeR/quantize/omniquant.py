...
import pdb
import gc
from quantize.utils import let_parameters, lwc_parameters, dynq_parameters, sv_parameters, get_omni_parameters,\
        ...
        if args.epochs > 0:
            ...
            optimizer = torch.optim.AdamW(
                [{"params":let_parameters(qlayer, use_shift),"lr":args.let_lr},
                 ...
                 {"params":dynq_parameters(qlayer), "lr":args.lwc_lr},
                 {"params":sv_parameters(qlayer), "lr":args.lwc_lr}],weight_decay=args.wd)
            loss_scaler = utils.NativeScalerWithGradNormCount()
            global_step = 0
            for epochs in range(args.epochs):
                loss_list = []
                norm_list = []
                # temperature = max(min_temperature, initial_temperature * (decay_rate ** epochs))
                for j in range(args.nsamples//args.batch_size):    
                # for j in range(4):    
                    index = j * args.batch_size
                    global_step += 1
                    # if epochs < args.epochs//4:
                    #     temperature = 1.
                    # else:
                    temperature = initial_temperature * (min_temperature / initial_temperature) ** (global_step / total_steps)
                    temperature = max(min_temperature, temperature)  # Ensure it doesn't go below min_temperature
                    ...
                    with traincast():
                        ...
                        for name, module in qlayer.named_modules():
                            if isinstance(module, QuantLinear):
                                module.weight_quantizer.temperature = temperature
                                module.act_quantizer.temperature = temperature
                ...
                logger.info(f"layer {i} iter {epochs} loss:{loss_mean} norm:{norm_mean} max memory_allocated {torch.cuda.max_memory_allocated(lm._device) / 1024**2} temp: {temperature} ")
