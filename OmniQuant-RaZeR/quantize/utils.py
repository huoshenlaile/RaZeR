...
def dynq_parameters(model):
    params = []
    for name, param in model.named_parameters():
        if 'quant_choice_param' in name:
            params.append(param)
    return iter(params)

def sv_parameters(model):
    params = []
    for name, param in model.named_parameters():
        if 'special_value' in name:
            params.append(param)
    return iter(params)
...
def omni_state_dict(model, destination=None, prefix='', keep_vars=False):
    ...
    for name, param in model.named_parameters():
        if name.find('smooth') > -1 or name.find('bound_factor') > -1 or name.find('quant_choice_param') > -1 or name.find('special_value') > -1:
