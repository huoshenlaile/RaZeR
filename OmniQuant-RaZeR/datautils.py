...
def get_c4(nsamples, seed, seqlen, model):
    ...
    for _ in range(nsamples):
        while True:
            ...
            if trainenc.input_ids.shape[1] >= seqlen + 1: # add extra 1 tokens to avoid short sequences. Sometimes the returned sequence length is exactly `seqlen`, which will cause an error in the following try block
                break
        try:
            i = random.randint(0, trainenc.input_ids.shape[1] - seqlen - 1)
    ...
    for _ in range(256):
        while True:
            ...
            if tmp.input_ids.shape[1] >= seqlen + 1: # add extra 1 tokens to avoid short sequences. Sometimes the returned sequence length is exactly `seqlen`, which will cause an error in the following try block
                break
