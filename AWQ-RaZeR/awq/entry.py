...
parser.add_argument("--eval_ppl", action="store_true", help="run perplexity experiments")
parser.add_argument("--wq_dtype", type=str, default=None, help="quantization datatype for weight") 
...
q_config = {
    ...
    "wq_dtype": args.wq_dtype, # quantization datatype
} 
    ...
    if args.eval_ppl:
        ...
        for task_eval in ["wikitext", "c4"]:
            if task_eval == "wikitext":
                ...
                random.seed(0)
                model_net = args.model_path.split('/')[-1]
                model_family = '_'.join(model_net.lower().split('-')[:-1])
                model.seqlen = args.seqlen

                cache_testloader = f'./data_cache//testloader_{model_family}_c4_{args.seqlen}.cache'
