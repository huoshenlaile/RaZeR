...
@torch.no_grad()
def auto_scale_block(module, module_kwargs, w_bit, q_config, input_feat, model):
    from .quantizer import pseudo_quantize_tensor

    # firstly, get the weight quantize function
    if w_bit is not None:

        def w_quantize_func(p, module_name):
    ...
    def _search_module_scale(block, linears2scale: list, x, kwargs={}):
        ...
        for ratio in range(n_grid):
            ...
            for fc in linears2scale:
                fc.weight.mul_(scales.view(1, -1).to(fc.weight.device))
                fc.weight.data = w_quantize_func(fc.weight.data, module_name=get_op_name(model, fc)) / (scales.view(1, -1))
            out = block(x, **kwargs)
