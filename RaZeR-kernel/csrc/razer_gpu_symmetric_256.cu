...
// same logic as v11, reduce one time size from 1024 to 256
...
__global__ void razer_gpu_symmetric_256_host(
    const float * __restrict__ A,        // [M, K] fp16 activations, row-major
    const unsigned int  * __restrict__ qB,     // [K_packed, N] int32, of 4-bit quantized values weights, column-major
    const float * __restrict__ scaling_factors, // [K /  group_size, N] fp16 scaling factors, column-major
    ...
    float* A_shared_float = reinterpret_cast<float*>(A_shared);

    __shared__ half scaling_factors_shared[8]; 
    float* scaling_factors_shared_float = reinterpret_cast<float*>(scaling_factors_shared);
    ...
    for (int k = 0; k < K_logical; k += 256){
        // load B matrix to shared memory
        unsigned int qb = qB[n * K_logical / 8 + k / 8 + threadIdx.x];

        float temp = A[m * K_logical / 2 + k / 2 + threadIdx.x + threadIdx.z * 32];
        __syncthreads();
        A_shared_float[threadIdx.x + threadIdx.z * 32] = temp;
        
        // load scaling factors to shared memory
        if (threadIdx.x == 0){
            scaling_factors_shared_float[threadIdx.z] = scaling_factors[(k / 256 + n * K_logical /256)]; //   ~ /128 / 8
            int mybitmods = bitmod[k / 1024 + n * k_bitmod_pack]; //   ~ /128 / 8
            int bitmod_offset = (k / 256) % 4;
            bitmod_shared[threadIdx.z] = mybitmods >> (bitmod_offset * 8);
        }
        __syncthreads();
        
        uint32_t myMod = (bitmod_shared[threadIdx.z] >> ((threadIdx.x / 16) * 4)) & 0x3;
        ...
        for (int i = 0; i < 8; i ++){
            ...
            float a = __half2float(A_shared[threadIdx.x * 8 + i]);
            acc_local += a * b;
        }

        acc += acc_local * __half2float(scaling_factors_shared[threadIdx.z * 2 + threadIdx.x / 16]);            
...
torch::Tensor razer_gpu_symmetric_256(
    ...
    const float* fA_ptr = reinterpret_cast<const float*>(fA.data_ptr<at::Half>());
    const unsigned int* qB_ptr = reinterpret_cast<const unsigned int*>(qB.data_ptr<int32_t>());
    const float* scaling_factors_ptr = reinterpret_cast<const float*>(scaling_factors.data_ptr<at::Half>());
    ...
    razer_gpu_symmetric_256_host<<<grid_size, block_size>>>(
        fA_ptr,
        qB_ptr,
        scaling_factors_ptr,
        bitmod_ptr,
