...
// ported from v11, one time size reduce from 1024 to 512
...
__global__ void razer_gpu_symmetric_512_host(
    const float2 * __restrict__ A,        // [M, K] fp16 activations, row-major
    const uint2  * __restrict__ qB,     // [K_packed, N] int32, of 4-bit quantized values weights, column-major
    const float2 * __restrict__ scaling_factors, // [K / group_size, N] fp16 scaling factors, column-major
    ...
    __shared__ half A_shared[512];
    float2* A_shared_float = reinterpret_cast<float2*>(A_shared);

    __shared__ half scaling_factors_shared[16]; 
    float2* scaling_factors_shared_float = reinterpret_cast<float2*>(scaling_factors_shared);
    ...
    int k_bitmod_pack = (K_logical / 128 + 7) / 8;

    for (int k = 0; k < K_logical; k += 512){
        // load B matrix to shared memory
        uint2 qb = qB[n * K_logical / 16 + k / 16 + threadIdx.x];

        float2 temp = A[m * K_logical / 4 + k / 4 + threadIdx.x + threadIdx.z * 32];
        __syncthreads();
        A_shared_float[threadIdx.x + threadIdx.z * 32] = temp;
        
        // load scaling factors to shared memory
        if (threadIdx.x == 0){
            scaling_factors_shared_float[threadIdx.z] = scaling_factors[(k / 512 + n * K_logical / 512)]; //   ~ /128 / 8
            int mybitmods = bitmod[k / 1024 + n * k_bitmod_pack]; //   ~ /128 / 8
            int bitmod_offset = (k / 512) % 2;
            bitmod_shared[threadIdx.z] = mybitmods >> (bitmod_offset * 16);
        }
        __syncthreads();
        
        uint32_t myMod = (bitmod_shared[threadIdx.z] >> ((threadIdx.x / 8) * 4)) & 0x3;
        ...
        for (int i = 0; i < 8; i ++){
            ...
            float a = __half2float(A_shared[threadIdx.x * 16 + i]);
            acc_local += a * b;
        }
        ...
        for (int i = 0; i < 8; i ++){
            ...
            float b = fp42float_v4(qb4, myMod);
            float a = __half2float(A_shared[threadIdx.x * 16 + i + 8]);
            acc_local += a * b;
        }


        acc += acc_local * __half2float(scaling_factors_shared[threadIdx.z * 4 + threadIdx.x / 8]);            
...
torch::Tensor razer_gpu_symmetric_512(
    ...
    const float2* fA_ptr = reinterpret_cast<const float2*>(fA.data_ptr<at::Half>());
    const uint2* qB_ptr = reinterpret_cast<const uint2*>(qB.data_ptr<int32_t>());
    const float2* scaling_factors_ptr = reinterpret_cast<const float2*>(scaling_factors.data_ptr<at::Half>());
    ...
    razer_gpu_symmetric_512_host<<<grid_size, block_size>>>(
        fA_ptr,
        qB_ptr,
        scaling_factors_ptr,
        bitmod_ptr,
